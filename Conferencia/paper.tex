...
\textit{Universidad Austral de Chile}\\
Valdivia, Chile \\
\texttt{luis.veasc@inf.uach.cl}}
\and
\IEEEauthorblockN{Christian Lazo}
\IEEEauthorblockA{\textit{Instituto de Informática}\\
\textit{Universidad Austral de Chile}\\
Valdivia, Chile \\
\texttt{clazo@inf.uach.cl}}
}
...
generalización para el tamaño del dataset. Se aplicó transfer learning a partir de pesos
preentrenados y fine-tuning sobre el dataset propio. Las imágenes de entrada fueron 
...
entrenamiento para ajustarse al límite de extensión del presente artículo. Este proceso 
de entrenamiento se ejecuta en el supercomputador Patagón de la Universidad Austral de Chile \cite{patagon},
y tuvo un tiempo de cómputo de 23 minutos con 28 segundos (0.391 horas) 
utilizando una GPU NVIDIA L40.
\begin{table}[!t]
...
Una vez seleccionada la detección de mayor confianza para cada letrero mediante el 
tracking por apariencia aplicado en la Etapa 2 de Implementación del
Modelo de Detección de Letreros, esta se recorta como ROI y se procesa mediante transformación a 
escala de grises y thresholding, técnica utilizada para mejorar el contraste previo 
al OCR \cite{thresholding-para-mejorar-ocr}. Dicho tracking opera sobre un fotograma 
por segundo del video, igual que los fotogramas enviados al modelo de detección, y 
asocia las detecciones de un mismo letrero mediante comparación de histogramas de 
color en el espacio HSV \cite{hsv-histogram-tracking, gil2004deteccion}. El reconocimiento de 
texto se realiza con PaddleOCR \cite{paddleocr,paddleocr2}, reteniendo resultados 
...
Para la tarea de detección se calcularon las métricas de Precisión, Recall y F1-score 
a partir de los verdaderos positivos (TP), falsos positivos (FP) y falsos negativos (FN), 
utilizando como ground truth \cite{ibm_ground_truth} un conteo manual frame a frame realizado durante la investigación, sobre la base de la taxonomía presentada en la 
Tabla~\ref{tab:taxonomia}. Para evitar el conteo múltiple de un mismo letrero detectado 
en frames consecutivos, se realizó un seguimiento visual manual de cada letrero de forma 
individual a lo largo del video, asociando todas sus detecciones a una única instancia 
para efectos de la evaluación. Estas métricas se definen según las Ecuaciones 
\ref{eq:precision}, \ref{eq:recall} y \ref{eq:f1}:
...
\begin{equation}
F1\text{-score} = 2 \cdot \frac{\text{Precisión} \cdot \text{Recall}}{\text{Precisión} + \text{Recall}}
\label{eq:f1}
\end{equation}

Para evaluar el desempeño del reconocimiento óptico de caracteres (OCR) se utilizó 
Character Error Rate (CER), calculado a partir de la distancia de edición de 
Levenshtein \cite{levenshtein1966binary,transkribus-cer} entre el texto reconocido 
por el modelo y el texto de referencia. Este texto de referencia se obtuvo mediante 
una transcripción manual del contenido textual de cada letrero identificado en el 
ground truth de detección, la cual fue comparada con la salida del OCR utilizando 
el algoritmo de Levenshtein, según se define en la ecuación \ref{eq:cer}:
...
\noindent donde $S$, $D$ e $I$ corresponden al número de sustituciones, eliminaciones 
e inserciones de caracteres necesarias para transformar el texto reconocido en el texto 
de referencia, y $N$ es el número total de caracteres del texto de referencia.

Para la interpretación de los resultados se definen rangos de referencia adaptados del 
estándar utilizado en plataformas de reconocimiento de texto \cite{transkribus-cer}. 
La Tabla~\ref{tab:cer_interpretacion} resume las categorías de interpretación 
utilizadas según el valor de CER obtenido.

\begin{table}[!t]
\caption{Interpretación de los valores de CER utilizados como referencia.}
\label{tab:cer_interpretacion}
\centering
\footnotesize
\begin{tabular}{cc p{3.6cm}}
\toprule
Rango de CER & Interpretación & Descripción \\
\midrule
...
\toprule
Video & Resolución & FPS & Precisión & Recall & F1-score & CER prom. & Interpretación prom. CER\\
\midrule
...
Algunos falsos positivos (FP) se producen por objetos rectangulares o planos, 
...
y 0,27, confirmando que la lectura de texto en escenas naturales y en movimiento
es la etapa más sensible del pipeline. En evaluación end-to-end, el pipeline completó 
automáticamente el flujo para el 57,1\% de los letreros turísticos 
correctamente detectados.

Con el fin de facilitar la revisión y reproducibilidad de este trabajo, el repositorio asociado al artículo,
que incluye el código principal empleado en esta investigación y en cuyo archivo README se encuentran los enlaces 
a los videos utilizados para este artículo y a los videos de prueba empleados para ajustar el modelo, 
disponible en el siguiente enlace:

\begin{center}
\textbf{\url{https://github.com/robleaustral/geodetector}}
\end{center}
...
datos geoespacial. También resulta relevante explorar edge computing para aproximar el pipeline a una ejecución en tiempo real desde el 
vehículo.
