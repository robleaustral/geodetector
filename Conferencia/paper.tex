...
pipeline híbrido que combina detección de letreros, extracción de
regiones de interés, reconocimiento óptico de caracteres, 
clasificación semántica por taxonomía turística, sincronización 
GPS y visualización georreferenciada. La propuesta fue evaluada con 
...
capturados principalmente en Chile y Argentina. En la evaluación 
experimental, el modelo alcanzó Precisión entre 0,77 y 0,91, Recall 
...
\begin{IEEEkeywords}
visión artificial, YOLO, OCR, georreferenciación, letreros turísticos, inteligencia artificial, rutas rurales.
\end{IEEEkeywords}
...
artesanales, deterioro físico, bajo contraste, oclusiones por 
vegetación, condiciones climáticas variables y captura desde 
...
letreros rurales, urbanos y turísticos; \textbf{(ii)} el diseño de un 
pipeline end-to-end que integra detección, OCR, clasificación 
semántica y geolocalización; y \textbf{(iii)} la evaluación 
...
Los datasets de señales de tránsito, como TT-100K y GTSDB, han permitido 
...
GEODETECTOR se organiza en dos etapas principales, como se muestra en la 
Fig.~\ref{fig:pipeline}. La primera etapa construye el modelo de detección
mediante un dataset etiquetado, su división en entrenamiento, validación y
prueba, y el fine-tuning de un modelo YOLO11m. La segunda etapa implementa 
...
los letreros. Cada imagen fue etiquetada manualmente con Label 
Studio \cite{labelstudio2026}, usando una clase única: \textit{signboard}. 
Las etiquetas fueron exportadas en formato TXT estándar de YOLO, y la 
metadata se consolidó mediante scripts en Python \cite{python2026}. Para 
resguardar privacidad, se difuminaron rostros, patentes, códigos QR y 
números telefónicos visibles mediante GIMP \cite{gimp2026}. El dataset fue 
...
\toprule
Región & Localidades principales & Imágenes \\
\midrule
...
generalización para el tamaño del dataset. Se aplicó transfer learning a 
partir de pesos preentrenados y fine-tuning sobre el dataset propio, con 
imágenes de entrada de 640$\times$640 píxeles, 50 épocas y parada temprana
con paciencia de 5 épocas, siguiendo la documentación de Ultralytics
\cite{yolo11_ultralytics}. La división de datos fue 70\% entrenamiento, 
15\% validación y 15\% prueba. La Tabla~\ref{tab:train_metrics} resume el 
...
La Fig.~\ref{fig:detecciones} muestra ejemplos de detecciones obtenidas 
...
definidos por organismos chilenos \cite{taxonomia-gobierno}. La comparación
usa coincidencia exacta y similitud difusa para tolerar errores de 
...
Pandas \cite{pandas} y se visualizan en un mapa interactivo.
...
actúa como el principal filtro del sistema. Aunque el detector identifica 
...
deterioro, àngulo de captura, reflejos o desenfoque por movimiento.

La Fig.~\ref{fig:endtoend} ilustra un caso exitoso de procesamiento 
...
Los resultados consolidados fueron representados en un mapa interactivo. 
La Fig.~\ref{fig:mapa} muestra la distribución geográfica de las 
...
La caída en Recall respecto del conjunto de validación era esperable, ya 
...
La Fig.~\ref{fig:limitaciones} muestra ejemplos de limitaciones observadas.
Algunos falsos positivos se producen por objetos rectangulares o planos, 
como laterales de camiones, banderas o estructuras viales. También se 
...
Lagos muestran que el detector alcanza Precisión entre 0,77 y 0,91, Recall 
entre 0,50 y 0,74 y F1-score entre 0,61 y 0,76. El OCR obtuvo valores de 
CER entre 0,08 y 0,27, confirmando que la lectura de texto en movimiento 
es la etapa más sensible del pipeline. En evaluación end-to-end, el sistema
completó automáticamente el flujo para el 57,1\% de los letreros turísticos
correctamente detectados.

Como trabajo futuro se proyecta ampliar el dataset con nuevas regiones, 
evaluar técnicas de super-resolución y corrección de desenfoque, 
